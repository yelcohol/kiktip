...
// 최신글 추출
// $cache_time 캐시 갱신시간
function latest($skin_dir='', $bo_table, $rows=10, $subject_len=40, $orders='', $cache_time=1, $options='')
{
    global $g5;
    ...
    if( $caches === false ){
        ...
        $sql = " select * from {$tmp_write_table} where wr_is_comment = 0 order by $orders limit 0, {$rows} ";
